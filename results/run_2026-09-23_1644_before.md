...
|---|---|---|---|
| When does dropping a course put a W on your transcript? | pass | pass | pass |
...
```
Dropping a course puts a W on your transcript if it is done after week two, and the dropping window runs through the end of week six. 
...
```
Dropping a course puts a W on your transcript if it is done after week two, up through the end of week six. 
...
```
Juniors and seniors in the housing lottery are ordered by accumulated credit hours first, with a random draw used only as a tie-breaker (admin_housing_lottery.txt).
```
...
```
Unused dining dollars roll over from the autumn semester to the spring semester, but whatever is left in May disappears and does not roll over to the following autumn (admin_dining_dollars.txt).
```
